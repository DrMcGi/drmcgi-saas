...
type Feature = { id: string; name: string; price: number; category: "website" | "saas" | "custom"; desc: string };
...
export default function Configurator() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-16 fade-up">
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="p-6 rounded-xl border border-white/10 bg-gray-800/40">
          ...
          <div className="mt-4">
            <div className="text-3xl bg-clip-text text-transparent" style={{ backgroundImage: "linear-gradient(90deg, var(--gold) 0%, var(--gold-soft) 100%)" }}>
              {fmt.format(totals.total)} ZAR
            </div>

            <button className="mt-2 text-xs text-white/60 hover:text-white transition" onClick={() => {
            ...
            <div id="breakdown" className="hidden mt-4 text-sm text-white/80 space-y-2">
              ...
              <div className="grid grid-cols-[1fr_auto] gap-3">
                <div>Subtotal</div><div className="text-white/60">{fmt.format(totals.base)} ZAR</div>
              </div>
              <div className="grid grid-cols-[1fr_auto] gap-3">
                <div>Premium</div><div className="text-white/60">{fmt.format(totals.premium)} ZAR</div>
              </div>
              <div className="border-t border-white/10" />
              <div className="grid grid-cols-[1fr_auto] gap-3 font-medium">
                <div>Total</div><div className="text-white">{fmt.format(totals.total)} ZAR</div>
              </div>
            </div>

            <p className="text-white/60 mt-3 text-xs">Tip: Packages auto‑add features; fine‑tune everything here.</p>
            <a href="#contact" className="mt-6 inline-block px-4 py-2 rounded-md bg-white text-gray-900 text-sm hover:opacity-90 transition">Request a blueprint</a>
